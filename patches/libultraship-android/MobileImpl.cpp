...
extern "C" void JNICALL Java_com_dishii_soh_MainActivity_setButton(JNIEnv* env, jobject obj, jint button,
                                                                     jboolean value) {
    if (button < 0) {
...
extern "C" void JNICALL Java_com_dishii_soh_MainActivity_nativeMenuNavKey(JNIEnv* env, jobject obj, jint dir,
                                                                            jboolean pressed) {
    if (dir >= 0 && dir < 6) {
        if (pressed) {
...
void Ship::Mobile::InjectMenuNavKeys() {
    ...
    for (int i = 0; i < 6; i++) {
        if (pressed & (1 << i))
            ImGui::GetIO().AddKeyEvent(kDirKeys[i], true);
        if (released & (1 << i))
            ImGui::GetIO().AddKeyEvent(kDirKeys[i], false);
    }
}
...
extern "C" void JNICALL Java_com_dishii_soh_MainActivity_setCameraState(JNIEnv* env, jobject obj, jint axis,
                                                                          jfloat value) {
    if (axis == 0) {
